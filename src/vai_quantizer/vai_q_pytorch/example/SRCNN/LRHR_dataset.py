...
class LRHRDataset(data.Dataset):
    ...
    def _load_file(self, idx):
        ...
        lr_yuv_img = cv.cvtColor(lr_img, cv.COLOR_BGR2YCrCb)
        hr_yuv_img =  cv.cvtColor(hr_img, cv.COLOR_BGR2YCrCb)
	
        lr_y_img = lr_yuv_img[:, :, 0]
        hr_y_img = hr_yuv_img[:, :, 0]
	
        #lr_y_img = lr_y_img/255.0
        #hr_y_img = hr_y_img/255.0
        
        lr_y_img = lr_y_img.astype(np.float32)
        hr_y_img = hr_y_img.astype(np.float32)
	
        lr_resize_y = cv.resize(lr_y_img, (lr_y_img.shape[1]*2, lr_y_img.shape[0]*2), interpolation = cv.INTER_CUBIC)


        return lr_resize_y, hr_y_img
