...
def ssim(img1, img2):
    ...
    img1 = img1.detach().numpy().astype(np.float64)
    img2 = img2.detach().numpy().astype(np.float64)
...
def calc_ssim(img1, img2):
    ...
    if img1.ndim == 4:
       ...
       for i in range(img1.shape[0]):
          vssim = ssim(processImage1[i,...], processImage2[i, ...])
          rssim.append(vssim)
    else:
...
def evaluate(model, val_loader, loss_fn):
  ...
  for iteraction, (lrimage, hrimage) in tqdm(
    ...
    for imgn in range(outputs.shape[0]):
      ...
      #cv2.imwrite(origin_image_name, origin_image.astype(np.uint8))
      #cv2.imwrite(process_image_name, process_Image.astype(np.uint8))
...
def quantization(title='optimize',
  ...
  input = torch.randn([batch_size, 1, 1024, 1280])
  if quant_mode == 'float':
    quant_model = model
    if inspect:
      if not target:
